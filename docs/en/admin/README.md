...
**`make install` → configure `.env` → `make start`**. The following sections detail
...
Then configure `.env` and run `make start` to build images on first start and launch Galaris.
